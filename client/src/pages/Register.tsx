...
interface RegisterProps extends Project {
  username: string;
  email: string;
  password: string;
}
...
function Register() {
  ...
  const onSubmit: SubmitHandler<Project> = async (data: Project) => {
    // console.log(data)
    try {
      const response = await axios.post(ApiConfig.API_REGISTER_URL, data, {
    ...
    <div className="w-full h-screen bg-white">
      <div className="w-full h-[60vh]">
        ...
        <div className="block h-[40vh] md:hidden">
          <img
            src="login-bg-1.svg"
            alt="Logo"
            className="w-full h-[30vh] object-cover z-10 opacity-85 rounded-bl-[6rem]"
          />
        </div>
      </div>
      <div className="w-full top-12 md:top-10 mx-auto mt-8 fixed z-10 flex flex-col justify-center items-center">
        ...
        <form
          ...
          className="bg-white w-full md:w-[30%] py-10 px-12 shadow-none md:shadow-lg rounded-lg"
        >
          ...
          >
            <label className="label">
              <span
                className={`label-text ${errors.email ? "text-red-500" : ""}`}
              >
                Email
              </span>
            </label>
          ...
          <div className="my-4 flex justify-end hover:cursor-pointer">
            <p className="text-sm text-blue-600 hover:text-700 transition-all">
              Forgot Password?
            </p>
          </div>
          ...
          <div className="flex justify-center items-center ">
            <button
              type="submit"
              className="btn bg-[#035FB2] hover:bg-[#045FB2] text-white btn-circle mt-6 w-full md:w-1/2 h-[0.5rem]"
            >
              Login
            </button>
          </div>
